...
<?= $this->section('title') ?><?= esc(lang('App.edit_trip_title')) ?><?= $this->endSection() ?>
...
<div class="page-card p-4 p-lg-5">
    <div class="mb-4">
        ...
        <h1 class="page-title h2 mb-2"><?= esc(lang('App.edit_trip_title')) ?></h1>
        <p class="muted mb-0"><?= esc(lang('App.edit_trip_desc')) ?></p>
    </div>
    ...
    <form action="<?= site_url('trips/update/' . (int) ($trip['id'] ?? 0)) ?>" method="post" id="trip-form" autocomplete="off">
        <?= csrf_field() ?>
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select class="form-select" id="ship_id" name="ship_id" required>
                    ...
                    <?php foreach ($ships as $ship): ?>
                        ...
                        <option value="<?= (int) ($ship['id'] ?? 0) ?>" data-label="<?= esc($shipLabel) ?>" <?= (int) old('ship_id', $trip['ship_id'] ?? 0) === (int) ($ship['id'] ?? 0) ? 'selected' : '' ?>>
                            <?= esc($shipLabel) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" id="departure_port_id" name="departure_port_id" required>
                    ...
                    <?php foreach ($ports as $port): ?>
                        <option value="<?= (int) ($port['id'] ?? 0) ?>" <?= (int) old('departure_port_id', $trip['departure_port_id'] ?? 0) === (int) ($port['id'] ?? 0) ? 'selected' : '' ?>>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" id="arrival_port_id" name="arrival_port_id" required>
                    ...
                    <?php foreach ($ports as $port): ?>
                        <option value="<?= (int) ($port['id'] ?? 0) ?>" <?= (int) old('arrival_port_id', $trip['arrival_port_id'] ?? 0) === (int) ($port['id'] ?? 0) ? 'selected' : '' ?>>
                            <?= esc($port['name'] ?? '') ?> — <?= esc(implode('، ', array_filter([$port['city'] ?? '', $port['country'] ?? '']))) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-md-3">
                ...
                <input type="datetime-local" class="form-control" id="departure_date" name="departure_date" min="<?= date('Y-m-d\TH:i') ?>" value="<?= esc(old('departure_date', date('Y-m-d\TH:i', strtotime((string) ($trip['departure_date'] ?? ''))))) ?>" required>
            </div>

            <div class="col-md-3">
                <label for="arrival_date" class="form-label"><?= esc(lang('App.arrival_date')) ?></label>
                <input type="datetime-local" class="form-control" id="arrival_date" name="arrival_date" min="<?= date('Y-m-d\TH:i') ?>" value="<?= esc(old('arrival_date', date('Y-m-d\TH:i', strtotime((string) ($trip['arrival_date'] ?? ''))))) ?>" required>
            </div>
        </div>
        ...
        <div class="d-flex gap-2 mt-4">
            <button type="submit" id="updateTripButton" class="btn btn-primary px-4">
                <i class="fa-solid fa-floppy-disk me-2"></i><span><?= esc(lang('App.update_trip')) ?></span>
            </button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const submitButton = document.getElementById('updateTripButton');
